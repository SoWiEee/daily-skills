...
完整案例：[`conversation.md`｜Case 6: paper-survey](conversation.md#case-6-paper-survey)。案例使用虛構的 CacheFormer 論文片段，示範如何從「三個 benchmark 有改善」收斂到較窄、而且證據支持得住的結論。
...
> 明天要和主管談兩個專案同時交付的問題，我想表達工作量太大，但不想聽起來像是在抱怨。
...
Skill 會用低門檻的問題開始，例如請你選擇要先整理「今天最累的地方」、「身體的感覺」或「今天有沒有一件值得記下來的事」。收尾可能整理成：

> 今天的我沒有解決所有事情，但有停下來看見自己真的累了。明天先留一小段不被打擾的時間給自己。
...
> 週六用 90 分鐘，不改題目，只用既有資料完成一頁「醜版本」，完成後請人看，再根據回饋決定下一步。
...
本次演練使用一段虛構的 CacheFormer 論文材料：三個 benchmark 的 exact-match 高於 baseline，平均 latency 從 150ms 降到 120ms，但缺少資料切分、洩漏控制、變異、信賴區間、完整 baseline 與硬體設定。因此結果只支持「這組實驗觀察到初步改善」，不能直接說成「普遍有效的解法」。完整研究地圖與 30 秒版本見 [`conversation.md`｜Case 6](conversation.md#case-6-paper-survey)。
